...
def upvoteFeedback(request, feedback_id) :
    ...
    if(isStudent(request)) :
        ...
        # if the student has already downvoted to this feedback and now he is upvoting,
            # we need to decrement the number of downvotes by 1, (that is given by this student),
        if not stu_fee_voting.downvotable :
...
def downvoteFeedback(request, feedback_id) :
    ...
    if(isStudent(request)) :
        stu_fee_voting = Voting.objects.filter(student=request.user.student, feedback=query_feedback).first()
        if not stu_fee_voting.upvotable :
            query_feedback.upvotes -= 1
            query_feedback.save()
